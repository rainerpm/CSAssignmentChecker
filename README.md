...
### Detecting Cheating
[PYTHON only for now] The **CSACcheat.py** program makes it easy to submit files submitted to CSAC to compare50 or moss for similarity checking. The program will submit all files that were judged to be correct and are in the assignments 00PLAGIARISM folder as well as any files that are in the **solutions** folder (this is were you can put common solutions to the problem that you suspect the students might be copying from).  **CSACcheat.py** assumes you've set up [compare50](https://cs50.readthedocs.io/projects/compare50/en/latest/) in the Windows Subsystem for Linux (wsl) and/or that you have repaced 11111111 in the **CSAClogin.txt** with your [moss](https://theory.stanford.edu/~aiken/moss/) userid.
