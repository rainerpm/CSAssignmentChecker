...
### Detecting Cheating
[PYTHON only for now] The **CSACcheat.py** program makes it easy to submit files submitted to CSAC to compare50 or moss for similarity checking. The program can also search for a provided regular expressions and analyse frequency of variable names. For compare50 & moss the program will submit all files that were judged to be correct and are thus in the assignment's **00PLAGIARISM** folder as well as any files that are in the assignment's **solutions** folder (this is were you can put common solutions to the problem that you suspect the students might be copying from).  **CSACcheat.py** assumes you've set up [compare50](https://cs50.readthedocs.io/projects/compare50/en/latest/) in the Windows Subsystem for Linux (wsl) and/or that you have repaced 11111111 in the **CSAClogin.txt** with your [moss](https://theory.stanford.edu/~aiken/moss/) userid.
